...
class Config(CLIParams, YamlModel):
    ...
    @model_validator(mode="after")
    def fix_enum_types(self):
        """Force api_type from string → LLMType enum"""
        if isinstance(self.llm.api_type, str):
            try:
                self.llm.api_type = LLMType(self.llm.api_type)
            except ValueError:
                raise ValueError(f"Invalid llm.api_type: {self.llm.api_type}")
        return self
